...
public class NuServerListActivity extends BaseActivity {

  private RecyclerView recyclerView;
  private List<ListItem> nuList = new ArrayList<>();
   ...
   * handling National University result options, and loads result links.
   *
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (getSupportActionBar() != null) {
      ...
      getSupportActionBar().setTitle(R.string.item_results);
    }
    ...
    adapter =
        new ItemAdapter(
            nuList,
            position -> {
              ListItem item = nuList.get(position);
              ItemModel model = (ItemModel) item;
              if (model.getUrl() == null
                  || model.getUrl().isEmpty()
                  || model.getUrl().startsWith("http") == false) {
                startActivity(new Intent(this, NuResultSmsActivity.class));
              } else {
  ...
  /** Fetches the dynamic list of National University result servers and local SMS options. */
  private void loadNuFromJson() {
    ...
    remoteConfigDataManager.loadData(
        new RemoteConfigDataManager.OnDataLoadedListener() {
          ...
          public void onDataLoaded(JSONObject configJson) {
            try {
              List<String> nuNames = remoteConfigDataManager.getNameList(configJson, "nu_result");
              List<String> nuUrls = remoteConfigDataManager.getUrlList(configJson, "nu_result");

              nuList.clear();
              for (int i = 0; i < nuNames.size(); i++) {
                nuList.add(
                    new ItemModel(
                        JsonNameTranslator.translateNameFromJson(context, nuNames.get(i)),
                        nuUrls.get(i)));
              }
              nuList.add(new ItemModel(getString(R.string.result_by_sms_title)));
