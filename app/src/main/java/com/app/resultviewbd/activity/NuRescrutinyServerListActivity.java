...
public class NuRescrutinyServerListActivity extends BaseActivity {
   ...
   * handling National University rescrutiny options, and loads rescrutiny links.
   *
    ...
    adapter =
        new ItemAdapter(
            ...
            position -> {
              ...
              if (item.getUrl().contains("result")) {
  ...
  /**
   * Fetches the dynamic list of National University rescrutiny server links from remote config
   * JSON.
   */
    ...
    remoteConfigDataManager.loadData(
        new RemoteConfigDataManager.OnDataLoadedListener() {
          ...
          public void onDataLoaded(JSONObject configJson) {
            try {
              serverList.clear();
              remoteConfigDataManager.addArrayItems(configJson, "nu_rescrutiny", serverList);
